...
struct TeleopTwistJoy::Impl
{
  ...
  ros::Publisher cmd_vel_pub; // usable to override other control modes
  ros::Publisher cmd_con_pub;
...
TeleopTwistJoy::TeleopTwistJoy(ros::NodeHandle *nh, ros::NodeHandle *nh_param)
{
  ...
  pimpl_->cmd_con_pub = nh->advertise<std_msgs::Bool>("control", 1, true);
  ...
  else
  {
    ...
    nh_param->param<double>("scale_linear", pimpl_->scale_linear_map["normal"]["x"], 3.0);
    nh_param->param<double>("scale_linear_turbo", pimpl_->scale_linear_map["turbo"]["x"], 4.0);
  }
  ...
  else
  {
    ...
    nh_param->param<double>("scale_angular", pimpl_->scale_angular_map["normal"]["yaw"], 2);
                                         ...
                                         const std::string &which_map)
{
  ...
  std_msgs::Bool cmd_con_msg;
  cmd_con_msg.data = true;

  cmd_vel_pub.publish(cmd_vel_msg);
  cmd_con_pub.publish(cmd_con_msg);
